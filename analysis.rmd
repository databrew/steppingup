...
get_body_weight_osduhs <- function(temp_dat){
  # temp <- unlist(lapply(strsplit(as.character(dat$hw_osduhs_weight), 
  #                         '/', 
  #                         fixed = TRUE), function(x){
  #                           x[1]
  #                         }))
  unique_pounds <- as.character(unique(sort(temp_dat$hw_osduhs_weight)))
  ...
  temp$demo_osduhs_new_weight <- ifelse(temp$sequence > 0 & temp$sequence <=6, '80_105',
  ...
  temp$sequence <- NULL
  temp_dat <- left_join(temp_dat, temp, by = c('hw_osduhs_weight' = 'pounds'))
  temp_dat$hw_osduhs_weight <- NULL
  return(temp_dat)
  
}

dat <- get_body_weight_osduhs(temp_dat = dat)
...
# get rid of 6th graders
dat <- dat %>% filter(demo_osduhs_grade != 'grade 6')

# relevel factors for interpretability
dat$demo_osduhs_grade <- relevel(dat$demo_osduhs_grade, ref = 'grade 7')
dat$demo_osduhs_new_weight <- relevel(as.factor(dat$demo_osduhs_new_weight), ref = '80_105')
...
# temp_demo <- dat[, grepl('^demo', colnames(dat))]
# temp_demo <- temp_demo[, apply(temp_demo, 2, function(x) length(unique(x)) < 10)]
...
slasso_results <- get_glm(model_data = dat, model_type = 'lasso', weights = NULL)
