...
get_glm <- function(model_data = dat, weights = NULL) {
  ...
  model_data <- model_data[, apply(model_data, 2, function(x) length(unique(x)) < 5)]
  # get feature matrix 
  outcome_data <- model_data[, grepl('^hw', colnames(model_data))]
  demo_data <- model_data[, !grepl('^hw', colnames(model_data))]
